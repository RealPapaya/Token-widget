...
- 開發/本機執行：`npm start`
- 不想看到終端機視窗時，可雙擊 `start-widget.vbs`。這只是隱藏啟動器，不是使用者需要理解的主要入口。

首次啟動會自動註冊「開機自動啟動」。之後可在 widget 內的設定頁或右鍵選單切換。
...
| `⟳` | 立即更新 |
| 齒輪設定頁 | 更新間隔、置頂、Windows 開機自動啟動、警示通知、顯示 Claude / Codex 用量、顯示 Amber Ladder |
| `⋯` 或右鍵 | 快速選單（置頂、開機啟動、警示通知、顯示 Claude / Codex 用量、顯示 Amber Ladder、結束） |
...
`%APPDATA%\claude-usage-widget\settings.json` — 視窗位置、面板寬度（`panelWidth`）、縮小狀態、置頂、開機啟動、通知、是否顯示 Claude 用量（`showClaude`，預設開啟）、是否顯示 Amber Ladder（`showAmberLadder`，預設關閉）、是否顯示 Codex 用量（`showCodex`，預設開啟）、更新間隔（`pollMinutes`）。
